...
function getChildrenOfNode(node: luaparse.Node): luaparse.Node[] {
  ...
  function addChild(n: luaparse.Node) {
    if (n && typeof n.type === 'string' && n.type !== 'Comment') {
      ...
      for (idx = children.length - 1; idx >= 0; --idx) {
        if (
          locStart(children[idx]) <= locStart(n) &&
  ...
  for (const key of keys) {
    const val = node[key];
    // @ts-ignore
    if (Array.isArray(val)) val.forEach(addChild);
    // @ts-ignore
    else if (val) addChild(val);
  }
...
export function attachComments(ast: luaparse.Chunk, options: Options) {
  for (const comment of ast.comments) {
    ...
    if (
      hasNewLine(options.sourceText, locStart(comment), {
      ...
      if (
        ...
        handleIfStatementsWithNoBodyComments(
          precedingNode,
          enclosingNode,
          followingNode,
          comment,
      ...
      if (
        ...
        handleDanglingIfStatementsWithNoBodies(
          precedingNode,
          enclosingNode,
          comment,
        )
...
function printTrailingComment(path: FastPath, options: Options) {
  ...
  if (
    hasNewLine(options.sourceText, locStart(comment), { searchBackwards: true })
  ) {
    const previousLineEmpty = isPreviousLineEmpty(
      options.sourceText,
      locStart(comment),
    );
  ...
  if (
    isNextLineEmpty(options.sourceText, locStart(comment), {
      searchBackwards: true,
    })
  ) {
    parts.push(hardline);
...
function addComment(node: luaparse.Node, comment: luaparse.Comment) {
  const comments =
    (node as any).attachedComments || ((node as any).attachedComments = []);
...
function addDanglingStatementComment(
  node: luaparse.Node,
  comment: luaparse.Comment,
) {
...
function handleStatementsWithNoBodyComments(
  enclosingNode: luaparse.Node,
  comment: luaparse.Comment,
) {
...
// itself.
function handleFunctionBodyComments(
  precedingNode: luaparse.Node,
  enclosingNode: luaparse.Node,
  comment: luaparse.Comment,
) {
  if (
  ...
  if (
    enclosingNode.parameters.length > 0 &&
    enclosingNode.parameters[enclosingNode.parameters.length - 1] ===
      precedingNode
  ) {
    addDanglingComment(enclosingNode, comment);
    return true;
...
function handleIfStatementsWithNoBodyComments(
  precedingNode: luaparse.Node,
  enclosingNode: luaparse.Node,
  followingNode: luaparse.Node,
  comment: luaparse.Comment,
) {
  ...
  if (
    followingNode &&
    (followingNode.type === 'ElseifClause' ||
      followingNode.type === 'ElseClause')
  ) {
...
function handleExpressionBeginComments(
  precedingNode: luaparse.Node,
  enclosingNode: luaparse.Node,
  comment: luaparse.Comment,
) {
  ...
  switch (enclosingNode.type) {
    ...
    case 'ForNumericStatement':
      if (
        precedingNode === enclosingNode.end ||
        precedingNode === enclosingNode.step
      ) {
        addDanglingStatementComment(enclosingNode, comment);
        return true;
      }
    ...
    case 'ForGenericStatement':
      if (
        precedingNode ===
        enclosingNode.iterators[enclosingNode.iterators.length - 1]
      ) {
        addDanglingStatementComment(enclosingNode, comment);
        return true;
      }
    ...
    case 'ElseifClause':
      ...
      if (
        precedingNode === enclosingNode.condition &&
...
function handleDanglingIfStatementsWithNoBodies(
  precedingNode: luaparse.Node,
  enclosingNode: luaparse.Node,
  comment: luaparse.Comment,
) {
