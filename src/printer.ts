...
function printStringLiteral(path: FastPath, options: Options): Doc {
  ...
  if (literal.type !== 'StringLiteral') {
    throw new Error(
      'printStringLiteral: Expected StringLiteral, got ' + literal.type,
    );
  }
  ...
  switch (node.type) {
    ...
    case 'AssignmentStatement': {
      ...
      left.push(
        indent(
          join(
            concat([',', shouldBreak ? hardline : line]),
      ...
      if (node.init.length) {
        ...
        if (node.init.length > 1) {
          right.push(
            indent(join(concat([',', line]), path.map(print, 'init'))),
          );
        } else {
        ...
        group(
          concat([
            ...
            indent(
              concat([
                softline,
                join(concat([',', line]), printedCallExpressionArgs),
              ]),
            ),
            softline,
            ')',
    ...
    case 'TableConstructorExpression':
      ...
      return group(
        concat([
          '{',
          indent(concat([softline, concat(fields)])),
          softline,
          '}',
        ]),
        shouldBreak,
...
export function buildDocFromAst(ast: luaparse.Chunk, options: Options) {
  const printNodeWithComments = (path: FastPath): Doc => {
    return printComments(path, options, p =>
      printNode(p, options, printNodeWithComments),
    );
  };
